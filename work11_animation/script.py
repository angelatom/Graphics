...
def first_pass( commands ):
    ...
    if frames and not basename:
        print 'Basename will be set to "name"'
...
def second_pass( commands, num_frames ):
    ...
    for command in commands:
        if command['op'] == 'vary':
            ...
            d = (float(args[3])-float(args[2])) / (float(args[1])-float(args[0]))
            x = command['args'][2]
            for i in range(start, end+1):
                frames[i][command['knob']] = x
                x += d
    return frames
        ...
        for command in commands:
            ...
            if c == 'box':
                ...
                if command['knob'] == None:
                    ...
                    if command['knob'] in frame:
                        k = frame[command['knob']]
            ...
            # end operation loop
        filename = 'anim/' + name + ('%03d'%counter)
